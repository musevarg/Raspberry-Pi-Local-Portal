...

{
    document.addEventListener("DOMContentLoaded", function () {
        var container = d3.select('.stacked-bar-chart__container');

        if (container[0][0]) {
            (function () {

                var testData = function testData(k) {
                    var data = [];
                    for (var i = 65; i <= 90; i++) {
                        data.push({
                ...
                nv.addGraph(function () {
                    var chart = nv.models.multiBarChart().margin({ top: 60, right: 20, bottom: 22, left: 50 }).reduceXTicks(true) //If 'false', every single x-axis tick label will be rendered.
                    .rotateLabels(0) //Angle to rotate x-axis labels.
                    .showControls(true) //Allow user to switch between 'Grouped' and 'Stacked' mode.
                    .groupSpacing(0.3) //Distance between each group of bars.
                    .stacked(true);

                    chart.xAxis.showMaxMin(false).tickSize(10).ticks(10);

                    chart.legend.margin({ top: 2, bottom: 10 });

                    chart.controls.margin({ top: 2, right: 0, bottom: 10, left: -18 }).color(["#FF0000", "#00FF00"]);
                    ...
                    container.append('svg').datum(data).call(chart);
